...
function doAuthenticate() {
    ...
    if (isset($_POST["authenticate"])&&($_POST['password']==''||$_POST['username']=='')){
	logout(_('Username of password fields must not be empty.'), 'error');
    }
...
function LDAPAuthenticate() {
    ...
    global $ldap_group_attribute;
    global $ldap_auto_register;
    global $default_perm_templ;
	...
	//givenName, sn, mail, 
        $ldapbind = ldap_bind($ldapconn, $ldap_binddn, $ldap_bindpw);
	//Besides the configured username attribute look for the user's first and last name & e-mail address
	$attributes=array($ldap_user_attribute, 'givenName', 'sn', 'mail');
	ldap_set_option($ldapconn, LDAP_OPT_REFERRALS, 0);
	ldap_set_option($ldapconn, LDAP_OPT_PROTOCOL_VERSION, $ldap_proto);
	$result=ldap_search($ldapconn, $ldap_basedn, $filter, $attributes);
	if(!$result) {
	    if (isset($_POST["authenticate"]) ) 
		log_error(sprintf('Failed LDAP user data fetch for user %s', $_SESSION["userlogin"]));
	    auth(_('Failed LDAP user data fetch'), "error");
	    return;
	}
	$userdata=ldap_get_entries($ldapconn, $result);
//	echo $userdata[0]['sn'][0].'<br />'.$userdata[0]['givenname'][0].'<br />'.$userdata[0]['mail'][0].'<br />'.$userdata[0]['samaccountname'][0].'<br />';
//	print_r($userdata);
        ...
        if (!$rowObj) {
	    //If the user's not registered yet (and auto LDAP registration is turned on), create the user
	    if($ldap_auto_register==1){
	        $query = "INSERT INTO users (username, password, fullname, email, description, perm_templ, active, use_ldap) VALUES ("
        	. $db->quote($userdata[0]['samaccountname'][0], 'text') . ", "
		. $db->quote('', 'text') . ", "
        	. $db->quote($userdata[0]['givenname'][0]." ".$userdata[0]['sn'][0], 'text') . ", "
        	. $db->quote($userdata[0]['mail'][0], 'text') . ", "
		. $db->quote('', 'text') . ", ".$default_perm_templ.", 1, 1)";
		$db->queryRow($query);
	    }
	    else{
            if (isset($_POST["authenticate"]) )
